...
def main():
    if constants.IS_REBUILD == 1:
        ...
        train = Dataset('data/raw_data/sdp_data_acentors_pos_synset_lower.train.txt', vocab_words=vocab_words, vocab_poses=vocab_poses, vocab_synset=vocab_synsets, vocab_depends=vocab_depends)
        pickle.dump(train, open(constants.PICKLE_DATA + 'train.pickle', 'wb'), pickle.HIGHEST_PROTOCOL)
        dev = Dataset('data/raw_data/sdp_data_acentors_pos_synset_lower.dev.txt', vocab_words=vocab_words, vocab_poses=vocab_poses, vocab_synset=vocab_synsets, vocab_depends=vocab_depends)
        pickle.dump(dev, open(constants.PICKLE_DATA + 'dev.pickle', 'wb'), pickle.HIGHEST_PROTOCOL)
        test = Dataset('data/raw_data/sdp_data_acentors_pos_synset_lower.test.txt', vocab_words=vocab_words, vocab_poses=vocab_poses, vocab_synset=vocab_synsets, vocab_depends=vocab_depends)
    ...
    props = ['words', 'positions_1', 'positions_2', 'labels', 'poses', 'synsets', \
             'relations', 'directions', 'identities']
    for prop in props:
